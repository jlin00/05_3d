...
struct matrix * generate_sphere(double cx, double cy, double cz,
  ...
  int i, j; //iterating purposes
  double rot, circ; //parametric purposes
  double x, y, z;
  for (i = 1; i <= step; i++){
    rot = 2 * M_PI * (double)i/step;
    for (j = 1; j <= step; j++){
      circ = M_PI * (double)j/step;
      x = r * cos(circ) + cx;
...
struct matrix * generate_torus( double cx, double cy, double cz,
  ...
  for (i = 1; i <= step; i++){
    rot = 2 * M_PI * (double)i/step;
    for (j = 1; j <= step; j++){
      circ = 2 * M_PI * (double)j/step;
      x = cos(rot) * (r1 * cos(circ) + r2) + cx;
      y =  r1 * sin(circ) + cy;
